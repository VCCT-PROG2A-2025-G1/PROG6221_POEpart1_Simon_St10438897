    ...
    | |__| (_) | | | | |_| |  | |_| |/ ___ \\__ \__ \  __/
     \____\___/|_| |_|\__|_|   \__, /_/   \_\___/___/\___|
       ...
       Cybersecurity Awareness Bot
        ...
        {
            ArrayList memory = new ArrayList(); //ArrayList to store the faveruite topics of the user
            ...
            while (true)
            {
                ...
                // Identify any recall command or topic
                if (question.Contains("recall") || question.Contains("remind me"))
                {
                    // find which topic they want to recall to double check if it does excist
                    var recallKey = MainResponces.responses.Keys.FirstOrDefault(k => question.Contains(k));

                    Console.WriteLine();
                    Console.WriteLine("----------------------------------------------------------------------------------------------------------------------");
                    Console.ForegroundColor = ConsoleColor.Green;
                    // Checks if the recallKey is not null and if it exists in the memory
                    //ChatGPT generated this code to check if the recallKey is in the memory
                    if (recallKey != null && memory.Contains(recallKey))
                    {
                        //Has already been covered and talked about before
                        if (MainResponces.memoryValue.TryGetValue(recallKey, out var recallMsg))
                        {
                            Console.WriteLine(recallMsg);
                        }
                    }
                    else if (recallKey != null)
                    {
                        //Has not been covered yet
                        Console.WriteLine($"Unfortunately we haven't covered '{recallKey}' as of yet but we can discuss it later!");
                    }
                    else
                    {
                        Console.WriteLine("I don't understand what you want me to recall, please try again.");
                        Console.WriteLine("Please enter 'remind me' or 'recall' with the topic you want to recall.");
                    }
                    Console.ResetColor();
                    Console.WriteLine("----------------------------------------------------------------------------------------------------------------------");
                    continue;
                ...
                if (matchKey != null)
                {
                    ...
                    {
                        //If the question is a new topic, add it to the memory
                        memory.Add(matchKey);
                    }
